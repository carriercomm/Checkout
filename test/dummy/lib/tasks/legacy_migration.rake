...
task :dbx2 => :environment do
  ...
  .each do |le|
    ...
    begin
      ...
      budget = Checkout::Budget.where(:number => number, :name => nom, :date_start => date_start, :date_end => date_end).first_or_initialize
  ...
  LegacyEquipment.includes(:legacy_budget).all.each do |le|
    ...
    begin
      ...
      # look up the model
      model_name = le.eq_model.blank? ? "Unknown" : le.eq_model.strip
      model_obj = maker.models.find_by_name(model_name)
      ...
      budget = Checkout::Budget.where(:number => number, :name => nom, :date_start => date_start, :date_end => date_end).first

      # find or create a matching asset tag
      asset_tag = Checkout::AssetTag.where(:uid => le.eq_uw_tag).includes(:part).first_or_initialize
