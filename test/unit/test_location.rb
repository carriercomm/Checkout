...
describe Location do
  ...
  it "should obey business hours" do
    base_time = DateTime.now.in_time_zone.at_beginning_of_day

    location = FactoryGirl.build_stubbed(:location, :name => "Republic of Vanuatu") do |loc|
      # add business hours every other day, with a lunch break in the middle
      [0, 48].each do |extra|
        # morning hours
        open_at  = base_time + 9.hours  + extra.hours
        close_at = base_time + 12.hours + extra.hours
        loc.business_hours.create(FactoryGirl.attributes_for(:business_hour, :open_at => open_at, :closed_at => close_at))

        # afternoon hours
        open_at  = base_time + 13.hours  + extra.hours
        close_at = base_time + 17.hours +  extra.hours
        loc.business_hours.create(FactoryGirl.attributes_for(:business_hour, :open_at => open_at, :closed_at => close_at))
      end

      # add one exception - make sure this exception falls on one of
      # our regular business hour days
      date_closed = (base_time + 48.hours).to_date
    ...
    # next week is open
    query_time = base_time + 50.hours + 1.week
